...
AEGIS_DECL LSW::v5::Tools::Future<message> message::edit(const std::string & content)
{
    ...
    assert(_channel != nullptr);
    if (_channel == nullptr)
        throw aegis::exception(make_error_code(error::channel_error));//return aegis::make_exception_future<message>(std::make_exception_ptr(aegis::exception(make_error_code(error::channel_error))));
